 ...
 *   5. Writes config.local.php with your DB credentials.
...
define('LOCAL_CONFIG', BASE_DIR . '/config.local.php');
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && (!$alreadyInstalled || $forced)) {
    ...
    if (!$errors) {
        try {
            ...
            // 4) Write config.local.php
            $tpl  = "<?php\n";
            $tpl .= "/* Generated by install.php on " . date('Y-m-d H:i:s') . ". */\n";
            ...
            if (file_put_contents(LOCAL_CONFIG, $tpl) === false) {
                throw new RuntimeException('Could not write config.local.php. Check folder permissions, or paste the credentials into config.php manually.');
            }
...
<div class="install-card">
  ...
  <?php else: ?>
    ...
    <form method="post" class="acard" action="install.php<?php echo $forced ? '?force=1' : ''; ?>">
      ...
      <div class="steps">The installer will import <code>database.sql</code>, create your admin account, write
        <code>config.local.php</code>, and lock itself when finished.</div>
